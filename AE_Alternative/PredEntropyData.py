...
class Data(torch.utils.data.Dataset):
    def __init__(self, data, type):  
        ...
        if type == 'long':
            self.data = [d.long() for d in data]
        else:
...
def build_pred_data(
    ...
    if nhs_files:
        if antibodies:
            data = Data([pred_patches, entropy_patches, dist_patches, nhs_patches, mt_patches, tom20_patches], type)
        else:
            data = Data([pred_patches, entropy_patches, dist_patches, nhs_patches], type)
    else:
        data = Data([pred_patches, entropy_patches, dist_patches], type)
    return data
